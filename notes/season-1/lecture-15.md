...
* Let's undertand the below code image and its explaination:
    ...
    * setTimeout(function cb() { //this calls the setTimeout web api which gives access to timer superpower. It registers the callback cb inside wep api envirinment and starts timer of 5 secs. console.log("Callback");}, 5000);
...
**Ans**: Suppose user clciks button x6 times. So 6 cb() are put inside callback queue. Event loop sees if call stack is empty/has space and whether callback queue is not empty(6 elements here). Elements of callback queue popped off, put in callstack, executed.
